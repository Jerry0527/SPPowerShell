...
Function Get-Permissions([Microsoft.SharePoint.Client.SecurableObject]$Object)
{
    ...
    Foreach($RoleAssignment in $RoleAssignments)
    {
        ...
        If($PermissionType -eq "SharePointGroup")
        {
            ...
            $GroupUsers = ($GroupMembers | Select -ExpandProperty Email) -join ","
        ...
        ElseIf($PermissionLevels -notcontains "Contribute; Read;")
        {
            #Add the Data to Object
            $Permissions = New-Object PSObject
            $Permissions | Add-Member NoteProperty Object($ObjectType)
            ...
            $Permissions | Add-Member NoteProperty URL($ObjectURL)
            $Permissions | Add-Member NoteProperty HasUniquePermissions($HasUniquePermissions)
            $Permissions | Add-Member NoteProperty Users($RoleAssignment.Member.TypedObject.Email)
            $Permissions | Add-Member NoteProperty Type($PermissionType)
            $Permissions | Add-Member NoteProperty Permissions($PermissionLevels)
...
Function Generate-SPOSitePermissionRpt()
{
    ...
    Try {
       ...
       $loginname="jerry@zheguo.onmicrosoft.com"
       $password = "*******"
       $secstr = New-Object -TypeName System.Security.SecureString
       $password.ToCharArray() | ForEach-Object {$secstr.AppendChar($_)}
       #$Cred= Get-Credential
       $Credentials = New-Object Microsoft.SharePoint.Client.SharePointOnlineCredentials($loginname, $secstr)
       ...
       $Ctx.Credentials = $Credentials
        ...
        $SiteCollectionAdmins = ($SiteAdmins | Select -ExpandProperty Email) -join ","
        #Add the Data to Object
        $Permissions = New-Object PSObject
...
$BatchSize = 500
$sites=Import-CSV "C:\MS Temp\FewSite.csv"
ForEach($site in $sites)
{
   $ReportFile="C:\Temp\"+ $site.Title +".csv"
   Generate-SPOSitePermissionRpt -SiteURL $site.FewSite -ReportFile $ReportFile
   #Generate-SPOSitePermissionRpt -SiteURL  $site.FewSite -ReportFile $ReportFile -Recursive -ScanItemLevel -IncludeInheritedPermissions
